...
export async function handleRetomar(
  ...
  await prisma.pontoEvent.create({
    data: { sessionId: open.id, type: PontoEventType.RETOMADA },
  });
  await prisma.pontoSession.update({
    where: { id: open.id },
    data: { status: PontoStatus.ABERTO },
  });

  await interaction.reply({ ephemeral: true, content: "▶️ Ponto retomado." });
}
